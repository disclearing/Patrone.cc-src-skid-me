        ...
        if (this.isInvulnerable(damagesource)) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if ((float) this.noDamageTicks > (float) this.maxNoDamageTicks / 2.0F) {
                    if (this.maxNoDamageTicks == 18) {
                        Entity entity = damagesource.getEntity();

                        if (entity instanceof EntityPlayer) {
                            PacketPlayOutEntityStatus packet = new PacketPlayOutEntityStatus(this, (byte) 2);
...
//                    flag = false;
    ...
    public void a(Entity entity, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            this.motX /= ToothlessConfig.knockbackFriction;
            this.motY /= ToothlessConfig.knockbackFriction;
            this.motZ /= ToothlessConfig.knockbackFriction;

            this.motX -= d0 / magnitude * ToothlessConfig.knockbackHorizontal;
            this.motY += ToothlessConfig.knockbackVertical;
            this.motZ -= d1 / magnitude * ToothlessConfig.knockbackHorizontal;

            if (this.motY > ToothlessConfig.knockbackVerticalLimit) {
                this.motY = ToothlessConfig.knockbackVerticalLimit;
            }
        }
    }
