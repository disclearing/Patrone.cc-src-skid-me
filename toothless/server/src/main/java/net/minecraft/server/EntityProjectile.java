...
public abstract class EntityProjectile extends Entity implements IProjectile {
    ...
    public void t_() {
        ...
        if (this.shooter instanceof EntityPlayer && this.shooter.getHealth() <= 0.0) {
            this.die();
            return;
        }
        ...
        // Toothless start - splash the potion after 4 ticks of living if it's in range of the player
        if (this instanceof EntityPotion && this.shooter != null && this.shooter.isSprinting() && ar > 5) {
            double diffX = Math.abs(this.shooter.locX - (this.locX + this.motX));
            double diffY = 2.0D - Math.abs(this.shooter.locY - (this.locY + this.motY));
            double diffZ = Math.abs(this.shooter.locZ - (this.locZ + this.motZ));

            if ((diffX + diffZ) / 2.0D < 0.75D && diffY > -0.35D) {
                this.die();
                this.a(new MovingObjectPosition(this.shooter));
