...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (this.b(packetplayinflying)) {
            ...
            if (!this.player.viewingCredits) {
                ...
                Location to = player.getLocation(); // Start off the To location as the Players current location.
                ...
                if (delta > 1.0F / 256 && checkMovement && !player.isDead()) {
                    for (MovementHandler handler : ToothlessServer.getInstance().getMovementHandlers()) {
                        handler.handleUpdateLocation(player, to, from, packetplayinflying);
                    }
                }

                if (deltaAngle > 1.0F / 256 && checkMovement && !player.isDead()) {
                    for (MovementHandler handler : ToothlessServer.getInstance().getMovementHandlers()) {
                ...
                if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
                    ...
                    if (!ToothlessConfig.disableMoveEvent) { // Spigot - don't skip any move events
    ...
    public void a(PacketPlayInBlockPlace packetplayinblockplace) {
        ...
        if (packetplayinblockplace.getFace() == 255) {
            ...
            if (reachDistance > (this.getPlayer().getGameMode() == GameMode.CREATIVE ? CREATIVE_PLACE_DISTANCE_SQUARED : SURVIVAL_PLACE_DISTANCE_SQUARED)) {
                return;
            }
    ...
    public void sendPacket(final Packet packet) {
        ...
        try {
            ...
            try {
                for (PacketHandler handler : ToothlessServer.getInstance().getPacketHandlers()) {
    ...
    public void a(PacketPlayInChat packetplayinchat) {
        ...
        if (this.player.dead || this.player.getChatFlags() == EntityHuman.EnumChatVisibility.HIDDEN) { // CraftBukkit - dead men tell no tales
            ...
            if (counted && chatSpamField.addAndGet(this, 20) > 100 && !this.minecraftServer.getPlayerList().isOp(this.player.getProfile())) { // Spigot
                if (!isSync) {
                    Waitable waitable = new Waitable() {
                        @Override
                        protected Object evaluate() {
    ...
    public void a(PacketPlayInArmAnimation packetplayinarmanimation) {
        ...
        if (!ToothlessConfig.practiceOptimizations) {
            float f1 = this.player.pitch;
            float f2 = this.player.yaw;
            double d0 = this.player.locX;
            double d1 = this.player.locY + (double) this.player.getHeadHeight();
...
//            }
    ...
    public void a(PacketPlayInKeepAlive packetplayinkeepalive) {
        if (packetplayinkeepalive.a() == this.i) {
            ...
            if (ToothlessConfig.proxyPipePingFix) {
                i /= 1.15;
            }
            this.player.ping = (this.player.ping + i) / 2;
        }
    }
    ...
    public void a(PacketPlayInTabComplete packetplayintabcomplete) {
        ...
        arraylist.addAll(this.minecraftServer.tabCompleteCommand(this.player, packetplayintabcomplete.a(), packetplayintabcomplete.b()));

        this.player.playerConnection.sendPacket(new PacketPlayOutTabComplete(arraylist.toArray(new String[0])));
    }
