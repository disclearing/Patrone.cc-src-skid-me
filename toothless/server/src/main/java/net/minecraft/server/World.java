...
public abstract class World implements IBlockAccess {
    ...
    public void makeSound(Entity entity, String s, float f, float f1) {
        ...
        for (IWorldAccess anU : this.u) {
            if (anU instanceof WorldManager) {
                ((WorldManager) anU).a(entity, s, entity.locX, entity.locY, entity.locZ, f, f1);
            } else {
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
        ...
        if ((j - i) * (l - k) * (j1 - i1) > 32 * 32 * 32) {
            MinecraftServer.LOGGER.warn(String.format("Filtered out large getCubes call %s,%s %s,%s %s,%s.", i, j, k, l, i1, j1));
    ...
    public void a(EntityHuman entityhuman, int i, BlockPosition blockposition, int j) {
        try {
            ...
            for (IWorldAccess anU : this.u) {
                if (anU instanceof WorldManager) {
                    ((WorldManager) anU).a(entityhuman, i, blockposition, j, true);
                } else {
                    anU.a(entityhuman, i, blockposition, j);
                }
            }
        } catch (Throwable throwable) {
